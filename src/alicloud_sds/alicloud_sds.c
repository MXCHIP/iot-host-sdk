...
mx_status alisds_init(const emh_alisds_config_t* config, int num_handles)
{
    ...
    require_action(num_handles > 0, exit, err = kGeneralErr);
...
void emh_ev_alisds_set_local_atts(emh_alisds_msg* attrs)
{
    ...
    for (int i = 0; i < num; i++) {
        ...
        if (alisds_attr_db[handle].write_func == NULL)
            continue;
...
{
    if (context.device_state != eState_M3_normal)
        return;
    ...
    if ((num == 0) || (attr_handles[0] == ALISDS_INVALID_HANDLE))
        return;
    ...
    for (i = 0; i < num && attr_handles[i] != ALISDS_INVALID_HANDLE; i++) {
        handle = attr_handles[i];
        if (alisds_attr_db[handle].read_func == NULL)
            continue;
    ...
    sds_log("Send to cloud %ld bytes > %s", strlen(buff), buff);
