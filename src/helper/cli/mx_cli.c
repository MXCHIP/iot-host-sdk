...
/// CLI ///
#define RX_WAIT 0xFFFFFFFF
#define SEND_WAIT 0xFFFFFFFF
        ...
        if (pCli.commands[i]->name != NULL) {
            if (!strncmp(inbuf, pCli.commands[i]->name, *bp)) {
                ...
                if (m == 1)
                    ...
                    cli_printf(" % s % s ", fm, pCli.commands[i]->name);
                else
                    cli_printf(" % s ", pCli.commands[i]->name);
            }
            n++;
        }
    }
...
static void help_command(char* pcWriteBuffer, int xWriteBufferLen, int argc,
    char** argv);
...
int cli_register_commands(const struct cli_command* commands,
    int num_commands)
{
    int i;
...
{
    if (msg[0] != 0)
        mx_hal_cli_putstr((const char*)msg, strlen(msg));
